            ...
            <div className="modal-action mt-8">
                <form method="dialog">
                    ...
                    <PDFDownloadLink document={<PDFDocument />} fileName={`SH_OD${InvoiceDetails.custmrDetails.cust_order_id}.pdf`}>
                        {({ blob, url, loading, error }) => (loading ? 'Loading document...' : <div className="btn mx-2 px-6 btn-sm bg-purple-600 hover:bg-purple-700 text-white font-medium py-3 rounded-lg transition-colors duration-200 shadow-sm border-none cursor-pointer">Download</div>
                        )}
                    </PDFDownloadLink>
                    
                    <button className="btn mx-2 px-6 btn-sm bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-3 rounded-lg transition-colors duration-200 border-none">cancel</button>
                </form>
            </div>
        </div>
    </dialog>
